...
describe("reports and archive",()=>{
 ...
 it("archives every terminal original byte-identically and includes cleaned/ files when present",async()=>{vi.setSystemTime(new Date("2026-01-01"));const bytes=new Uint8Array([255,216,255,7,8]),cleanedBytes=new Uint8Array([255,216,255,9,10]),queuedItem=item("1","ảnh.jpg","review",bytes);queuedItem.cleanedFile=new File([cleanedBytes],"ảnh-cleaned.jpg",{type:"image/jpeg"});const items=[queuedItem,item("2","ảnh.jpg","cancelled",bytes)];const zip=await JSZip.loadAsync(await createArchive(items));expect(new Uint8Array(await zip.file("originals/ảnh.jpg")!.async("arraybuffer"))).toEqual(bytes);expect(new Uint8Array(await zip.file("originals/ảnh (2).jpg")!.async("arraybuffer"))).toEqual(bytes);expect(new Uint8Array(await zip.file("cleaned/ảnh-cleaned.jpg")!.async("arraybuffer"))).toEqual(cleanedBytes);expect(JSON.parse(await zip.file("report.json")!.async("string")).records).toHaveLength(2);});
});
